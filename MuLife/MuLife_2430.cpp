...
void plotTauVsBinning(const std::vector<double>& data, int rangeStart, int rangeEnd) {
    std::vector<int> binCounts = {75, 125, 200, 275, 350, 415, 500};
...
int sceltabinning() {
    // Apri il file ROOT
    TFile* inputFile = TFile::Open("time_differences_2430.root", "READ");
...
int main() {
    // Apri il file ROOT
    TFile* inputFile = new TFile("time_differences_2430.root", "READ");
    if (!inputFile || inputFile->IsZombie()) {
        std::cerr << "Errore: impossibile aprire il file ROOT!" << std::endl;
        return 1;
    }
    ...
    canvas1->Draw();
    gStyle->SetOptFit(1111);
    canvas1->SaveAs("Risultati/Vita media/Fit_2430_bin100.jpg");
    canvas1->SaveAs("Risultati/Vita media/Fit_2430_bin100.pdf");
