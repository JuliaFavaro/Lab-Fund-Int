...
#include <TMath.h>
#include <iomanip> // Includes std::setprecision

//function to determine the uncertanty on the efficiency
...
void calibration0() {
    ...
    for (int k = 0; k < n1; k++) {
        ...
        Epsilon1_acc->SetPointError(k, 0, sigma_eff(counts123_e1[k],counts23[k], acc23,150*30, 140*30, 30, acc123));//binomial error if we overlook the accidental counts
    ...
    std::cout << voltage1[6] << " Singola " 
            << counts1[6] / time << " " 
            << sqrt(counts1[6]) / time 
            << " Efficienza " 
            << eff1_acc[6] << " " 
            << sigma_eff(counts123_e1[6], counts23[6], acc23, 150*30, 140*30, 30, acc123) 
            << std::endl;

    for (int k = 0; k < n2; k++) {
        eff2_acc[k] = counts123_e2[k] / (counts13[k] - acc13-acc123);
        Epsilon2_acc->SetPoint(k, voltage2[k], eff2_acc[k]);
        Epsilon2_acc->SetPointError(k, 0, sigma_eff(counts123_e2[k],counts13[k], acc13,140*30, 103*30, 30, acc123));//binomial error if we overlook the accidental counts
    ...
    std::cout << voltage2[7] << " Singola " 
            ...
            << " Efficienza "
            << eff2_acc[7] << " "
            << sigma_eff(counts123_e2[7],counts13[7], acc13,140*30, 103*30, 30, acc123)<< std::endl;

    for (int k = 0; k < n3; k++) {
        eff3_acc[k] = counts123_e3[k] / (counts12[k] - acc12-acc123);
        Epsilon3_acc->SetPoint(k, voltage3[k], eff3_acc[k]);
        Epsilon3_acc->SetPointError(k, 0, sigma_eff(counts123_e3[k],counts12[k], acc12,150*30, 103*30, 30, acc123));//binomial error if we overlook the accidental counts
    ...
    std::cout << voltage3[5] << " Singola " 
            ...
            << " Efficienza " 
            << eff3_acc[5] << " " 
            << sigma_eff(counts123_e3[5],counts12[5], acc12,150*30, 103*30, 30, acc123)<< std::endl;
    ...
    c1->SaveAs("Risultati/Puntilavoro_08.png");
}