...
TF1* calibration_exp(){
    ...
    fExp1Gaus->SetParameters(p0, p1, 16000, 2420, 81);
...
TF1* calibration_pol5(){
    ...
    // Fit 2: Fondo polinomiale + prima gaussiana
    TF1* fPol1Gaus = new TF1("fPol1Gaus", "[0] + [1]*x + [2]*x*x + [3]*x*x*x + [4]*x*x*x*x + [5]*x*x*x*x*x + [6]*exp(-0.5*((x-[7])/[8])**2)", 1600, 2800);
    fPol1Gaus->SetParameters(5, -0.001, 1e-7, -1e-11, 1e-15, -1e-19, 16000, 2420, 40); // Ampiezza da immagine intorno a 70, centro 1800, sigma 100

    //fPol1Gaus->SetParLimits(6, 15960, 16040); // Vincola la media del ampiezza picco
    //fPol1Gaus->SetParLimits(8,  30, 50); // Vincola la media del larghezza picco
    
    fPol1Gaus->SetParName(6, "A_1"); //ampiezza gaussiana
    fPol1Gaus->SetParName(7, "#mu_1"); 
    fPol1Gaus->SetParName(8, "#sigma_1");
    hCs->Fit("fPol1Gaus","L","", 1600, 2800);
